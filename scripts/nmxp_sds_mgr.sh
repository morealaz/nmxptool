...
while [ $JDAY -le ${JDAYEND} ]; do
	...
	for STATION in ${STATION_LIST}; do
		STA=`echo ${STATION} | cut -f 1 -d'.'`
		CHAN=`echo ${STATION} | cut -f 2 -d'.'`
		NET=`echo ${STATION} | cut -f 3 -d'.'`
